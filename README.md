...
instructions at the [bottom of this page](#using-smallish-as-a-template).
...
`smallish/smallish.py`. The code provided in this file is intentionally rather
...
We place the module code in a file called `smallish.py` in directory called
`smallish`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import smallish as sb` in an
interactive Python session, the classes and functions defined inside of the
`smallish.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the smallish module, the main classes defined at the bottom of
...
Either way, you can create a `smallish/data` folder in which you can
...
(typically, something like our `smallish/tests/test_smallish.py`). Within each
...
have one such test in `smallish/tests/test_smallish.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/smallish`),
and type:
...
will see a message such as:
    ...
    smallish/tests/test_smallish.py .F...
    ...
    smallish/tests/test_smallish.py:49: AssertionError
...
To document `smallish` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/smallish/blob/master/doc/_static/RTD-advanced-conf.png)

 http://smallish.readthedocs.org/en/latest/
...
A `smallish/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/smallish) for the software. This
...
`smallish` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `smallish`, we use the
...
[here](https://github.com/uwescience/smallish/blob/master/scripts/Figure1.ipynb).
...
Under "old repository clone URL" enter: https://github.com/uwescience/smallish
...
`smallish/smallish.py` to be called `smallish/smallish.py`

	git mv smallish smallish
	git mv smallish/smallish.py smallish/smallish.py
	git mv smallish/tests/test_smallish.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `smallish` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `smallish` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `smallish` in them. Type
the following to see where all these files are:

	git grep smallish

You can replace `smallish` for `smallish` quickly with:

	git grep -l 'smallish' | xargs sed -i 's/smallish/smallish/g'
...
Edit all the mentions of `smallish` in the `smallish/__init__.py` file, and in
the `smallish/version.py` file as well.
